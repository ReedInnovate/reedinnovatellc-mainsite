  ...
  return <AnimatedSection id="you" className="bg-gray-50 py-16 md:py-24">
      <div className="container mx-auto px-4 md:px-6">
        <div className="text-left md:text-center mb-10 md:mb-16">
          ...
          <p className="section-description text-center md:text-left text-lg md:text-xl">
              ...
              <div className="flex flex-col h-full">
                ...
                <div className="mb-6">
                  ...
                  <div className="h-3 md:h-4 bg-gray-100 rounded-full mt-3 overflow-hidden relative">
                    <div className="h-full bg-primary transition-all duration-200" style={{
                    width: `${progress}%`
                  }} />
                    {/* Custom rocket image that moves with the progress */}
                    <div className="absolute top-1/2 transition-all duration-200" style={{
                    left: `${progress}%`,
                    transform: `translateX(-50%) translateY(-50%)`,
                    height: '30px',
                    width: '24px'
                  }}>
                      <img src="/lovable-uploads/ac00de8e-a6b4-4788-aad3-5c221ef66148.png" alt="Rocket" className="h-full w-full object-contain rotate-90" />
                    </div>
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center mt-auto">
                  {stages.map(stage => <div key={stage.title} className={cn("flex flex-col items-center transition-all duration-300", stage.title === currentStage ? "scale-110" : "opacity-60")}>
                      <div className={cn("w-4 h-4 md:w-5 md:h-5 rounded-full flex items-center justify-center mb-1", stage.title === currentStage ? "bg-primary" : "bg-gray-200")}>
                        {stage.title === currentStage && <div className="w-1 h-1 md:w-1.5 md:h-1.5 bg-white rounded-full" />}
                      </div>
                      <span className={cn("text-sm md:text-base font-medium capitalize", stage.title === currentStage ? "text-primary" : "text-gray-500")}>
                        {stage.title}
                      </span>
                    </div>)}
                </div>
